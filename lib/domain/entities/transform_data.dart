...
class TransformData {
  const TransformData({this.scale = 1.0, this.rotation = 0.0, this.translate = Offset.zero});

  final double rotation, scale;
  final Offset translate;

  TransformData copyWith({double? scale, double? rotation, Offset? translate}) => TransformData(
  ...
  factory TransformData.fromRect(
    ...
    return TransformData(rotation: rotation, scale: scale, translate: translate);
  }

  factory TransformData.fromController(VideoEditorController controller) {
    return TransformData(rotation: -controller.cacheRotation * (pi / 180.0), scale: 1.0, translate: Offset.zero);
  }
}
