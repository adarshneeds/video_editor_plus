...
import 'package:video_editor_plus/widgets/trim/thumbnail_slider.dart';
import 'package:video_editor_plus/widgets/trim/trim_slider_painter.dart';
...
class _TrimSliderState extends State<TrimSlider> with AutomaticKeepAliveClientMixin<TrimSlider> {
  ...
  void attachTrimToScroll() {
    if (_scrollController.position.outOfRange == false) {
      ...
      if (_scrollController.offset == 0.0) {
        _changeTrimRect(left: _rect.left - _lastScrollPixels.abs(), updateTrim: false);
      } else if (_scrollController.offset == _scrollController.position.maxScrollExtent) {
        _changeTrimRect(left: _rect.left + (_lastScrollPixels.abs() - _scrollController.offset), updateTrim: false);
      }
      ...
      if (_scrollController.position.extentBefore == 0.0 && _preSynchLeft == null) {
        ...
        final scrollOffset =
            (_scrollController.position.maxScrollExtent - (_lastScrollPixelsBeforeBounce ?? _scrollController.offset))
                .abs();
        _preSynchRight = max(0, _trimLayout.width - (_rect.right - _horizontalMargin) - scrollOffset);
      }
      _lastScrollPixelsBeforeBounce = null;
    }
    ...
    if (_scrollController.position.extentAfter == 0.0 &&
      ...
      _changeTrimRect(left: -_scrollController.offset + _horizontalMargin + (_preSynchLeft ?? 0), updateTrim: false);
    }
  ...
  void _onHorizontalDragUpdate(DragUpdateDetails details) {
    ...
    switch (_boundary) {
      ...
      case _TrimBoundaries.right:
        ...
        _changeTrimRect(
          width:
              (_rect.left + _rect.width + delta.dx).clamp(_rect.left, _trimLayout.width + _horizontalMargin) -
              _rect.left,
        );
        break;
      case _TrimBoundaries.inside:
        if (_isExtendTrim) {
          _scrollController.position.moveTo(_scrollController.offset - delta.dx, clamp: false);
        } else {
          // avoid rect to be out of bounds
          _changeTrimRect(
            left: posLeft.dx.clamp(_horizontalMargin, _trimLayout.width + _horizontalMargin - _rect.width),
          );
        }
        break;
      case _TrimBoundaries.progress:
        ...
        _controllerSeekTo((pos + localAdjust).clamp(_rect.left - _horizontalMargin, _rect.right + _horizontalMargin));
        break;
      default:
        break;
    }
  ...
  void _updateControllerTrim() {
    widget.controller.updateTrim(_getRectToTrim(_rect.left), _getRectToTrim(_rect.right));
    _resetControllerPosition();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (_, contrainst) {
        final Size trimLayout = Size(contrainst.maxWidth - _horizontalMargin * 2, contrainst.maxHeight);
        _fullLayout = Size(trimLayout.width * (_isExtendTrim ? _viewportRatio : 1), contrainst.maxHeight);
            ...
            children: [
                ...
                child: SingleChildScrollView(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      children: [
                        ClipRRect(
                          borderRadius: BorderRadius.circular(widget.controller.trimStyle.borderRadius),
                          child: SizedBox(
              ...
              GestureDetector(
                ...
                child: AnimatedBuilder(
                  animation: Listenable.merge([widget.controller, widget.controller.video]),
                  builder: (_, __) {
