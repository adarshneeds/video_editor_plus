...
import 'package:video_editor_plus/widgets/crop/crop_grid_painter.dart';
import 'package:video_editor_plus/widgets/image_viewer.dart';
import 'package:video_editor_plus/widgets/transform.dart';

class ThumbnailSlider extends StatefulWidget {
  const ThumbnailSlider({super.key, required this.controller, this.height = 60, this.quality = 10});
...
class _ThumbnailSliderState extends State<ThumbnailSlider> {
  ...
  Stream<List<Uint8List>> _generateThumbnails() =>
      generateTrimThumbnails(widget.controller, quantity: _thumbnailsCount, quality: widget.quality);
                ...
                ? ListView.builder(
                    ...
                    itemBuilder: (_, i) => ValueListenableBuilder<TransformData>(
                      ...
                      builder: (_, transform, __) {
                        ...
                        return Stack(
                          children: [
                            _buildSingleThumbnail(data[0], transform, isPlaceholder: true),
                            if (index < data.length)
                              _buildSingleThumbnail(data[index], transform, isPlaceholder: false),
                          ],
                        );
                      },
                    ),
  ...
  Widget _buildSingleThumbnail(Uint8List bytes, TransformData transform, {required bool isPlaceholder}) {
