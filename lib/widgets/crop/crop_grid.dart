...
import 'package:video_editor_plus/models/transform_data.dart';
import 'package:video_editor_plus/config/helpers.dart';
import 'package:video_editor_plus/widgets/crop/crop_mixin.dart';
import 'package:video_editor_plus/widgets/video_viewer.dart';
...
class CropGridViewer extends StatefulWidget {
  ...
  const CropGridViewer.preview({super.key, required this.controller})
      : showGrid = false,
        rotateCropArea = true,
...
class _CropGridViewerState extends State<CropGridViewer> with CropPreviewMixin {
  ...
  double? get aspectRatio =>
      widget.rotateCropArea == false && _controller.isRotated && _controller.preferredCropAspectRatio != null
          ? getOppositeRatio(_controller.preferredCropAspectRatio!)
          : _controller.preferredCropAspectRatio;

  Size _computeLayout() =>
      computeLayout(_controller, margin: widget.margin, shouldFlipped: _controller.isRotated && widget.showGrid);
  ...
  Offset get gestureOffset =>
      Offset((viewerSize.width / 2) - (layout.width / 2), (viewerSize.height / 2) - (layout.height / 2));
  ...
  void _onPanEnd({bool force = false}) {
    if (_boundary != CropBoundaries.none || force) {
      ...
      _controller.cacheMinCrop = Offset(r.left / layout.width, r.top / layout.height);
      _controller.cacheMaxCrop = Offset(r.right / layout.width, r.bottom / layout.height);
  ...
  void _changeRect({double? left, double? top, double? right, double? bottom}) {
    ...
    if (newRect.width < minRectSize || newRect.height < minRectSize || !isRectContained(layout, newRect)) return;
  ...
  Widget _buildCropView(TransformData transform) {
    return Padding(
      ...
      child: buildVideoView(_controller, transform, _boundary, showGrid: widget.showGrid),
    );
  }
}
