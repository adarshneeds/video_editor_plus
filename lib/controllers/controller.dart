...
import 'package:video_editor_plus/models/cover_data.dart';
import 'package:video_editor_plus/models/cover_style.dart';

import 'package:video_editor_plus/models/crop_style.dart';
import 'package:video_editor_plus/ffmpeg/ffmpeg_command_config.dart';
import 'package:video_editor_plus/models/trim_style.dart';
import 'package:video_editor_plus/config/helpers.dart';
import 'package:video_editor_plus/config/thumbnails.dart';
...
class VideoEditorController extends ChangeNotifier {
  ...
  VideoEditorController.file(
    ...
    this.defaultCoverThumbnailQuality = 10,
        ...
        trimStyle = trimStyle ?? TrimSliderStyle(background: Colors.transparent),
  ...
  Size get croppedArea =>
      Rect.fromLTWH(0, 0, videoWidth * (maxCrop.dx - minCrop.dx), videoHeight * (maxCrop.dy - minCrop.dy)).size;
  ...
  String _getCrop() {
    ...
    int x = startdx.floor();
    int y = startdy.floor();
    int w = (enddx - startdx).floor();
    int h = (enddy - startdy).floor();

    // Clamp to source bounds
    final int vidW = videoWidth.floor();
    final int vidH = videoHeight.floor();
    if (w > vidW - x) w = vidW - x;
    if (h > vidH - y) h = vidH - y;

    // Enforce even dims (safer for 4:2:0)
    if ((w & 1) == 1) w -= 1;
    if ((h & 1) == 1) h -= 1;

    // Ensure minimally valid size
    if (w < 2) w = 2;
    if (h < 2) h = 2;

    // Re-clamp origin after adjustments
    if (x > vidW - w) x = vidW - w;
    if (y > vidH - h) y = vidH - h;
    if (x < 0) x = 0;
    if (y < 0) y = 0;

    // ➜ exact=1 avoids internal rounding that can cause edge artifacts
    return "crop=$w:$h:$x:$y:exact=1";
  }
  ...
  void updateTrim(double min, double max) {
    ...
    assert(
      newDuration <= maxDuration && newDuration >= minDuration,
  ...
  VideoFFmpegCommandConfig createVideoFFmpegConfig() {
    return VideoFFmpegCommandConfig(trimCommand: _trimCmd, crop: _getCrop(), rotation: _getRotation());
  }

  CoverFFmpegCommandConfig createCoverFFmpegConfig() {
    return CoverFFmpegCommandConfig(crop: _getCrop(), rotation: _getRotation());
  }
}
