...
import 'package:video_editor_plus/domain/entities/trim_style.dart';

class TrimSliderPainter extends CustomPainter {
  const TrimSliderPainter(this.rect, this.position, this.style, {this.isTrimming = false, this.isTrimmed = false});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final rrect = RRect.fromRectAndRadius(rect, Radius.circular(style.borderRadius));
  ...
  void paintBar(
    ...
    canvas.drawPath(
      Path()
        ..addRect(Rect.fromPoints(rect.topLeft, rect.topRight - Offset(0.0, style.lineWidth)))
        ..addRect(Rect.fromPoints(rect.bottomRight + Offset(0.0, style.lineWidth), rect.bottomLeft)),
      edges,
    );
  ...
  void paintEdgesBarPath(
    ...
    if (style.borderRadius == 0) {
      canvas.drawPath(
        Path()
          // LEFT EDGE
          ..addRect(
            Rect.fromCenter(center: centerLeft, width: style.edgesSize, height: size.height + style.lineWidth * 2),
          )
          // RIGTH EDGE
          ..addRect(
            Rect.fromCenter(center: centerRight, width: style.edgesSize, height: size.height + style.lineWidth * 2),
          ),
        edges,
      );
    }
    ...
    canvas.drawPath(
      ...
      Path()
        ...
        ..addRRect(
          RRect.fromRectAndCorners(
            Rect.fromLTWH(centerLeft.dx + halfLineWidth, 0.0, style.borderRadius, size.height),
            topLeft: Radius.zero,
    ...
    canvas.drawPath(
      ...
      Path()
        ...
        ..addRRect(
          RRect.fromRectAndCorners(
            Rect.fromLTWH(centerRight.dx - halfLineWidth - style.borderRadius, 0.0, style.borderRadius, size.height),
    ...
    canvas.drawRRect(
      RRect.fromRectAndRadius(
        Rect.fromPoints(
          ...
          Offset(position + style.positionLineWidth / 2, size.height + style.lineWidth * 2),
        ),
  ...
  void paintIcons(Canvas canvas, {required Offset centerLeft, required Offset centerRight}) {
    ...
    if (style.leftIcon != null) {
      ...
      leftArrow.text = TextSpan(
        ...
        style: TextStyle(fontSize: style.iconSize, fontFamily: style.leftIcon!.fontFamily, color: style.iconColor),
      );
      leftArrow.layout();
    ...
    if (style.rightIcon != null) {
      ...
      rightArrow.text = TextSpan(
        ...
        style: TextStyle(fontSize: style.iconSize, fontFamily: style.rightIcon!.fontFamily, color: style.iconColor),
      );
      rightArrow.layout();
