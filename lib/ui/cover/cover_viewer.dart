...
class CoverViewer extends StatefulWidget {
  ...
  const CoverViewer({super.key, required this.controller, this.noCoverText = 'No selection'});
...
class _CoverViewerState extends State<CoverViewer> with CropPreviewMixin {
  ...
  void _scaleRect() {
    ...
    transform.value = TransformData.fromRect(rect.value, layout, viewerSize, widget.controller);
  ...
  Widget buildView(BuildContext context, TransformData transform) {
    return ValueListenableBuilder(
      ...
      builder: (_, CoverData? selectedCover, __) {
        ...
        return buildImageView(widget.controller, selectedCover!.thumbData!, transform);
      },
    );
  }
}
