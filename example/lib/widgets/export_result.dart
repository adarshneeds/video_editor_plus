...
class _VideoResultPopupState extends State<VideoResultPopup> {
  ...
  late final _isGif =
      kIsWeb ? widget.video.mimeType == 'image/gif' : path.extension(widget.video.path).toLowerCase() == ".gif";
  String? _fileMbSize;
        ...
        child: Stack(
          ...
          children: [
            AspectRatio(
              ...
              child: _isGif
                  ? (kIsWeb ? Image.network(widget.video.path) : Image.file(File(widget.video.path)))
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Center(
        child: Stack(
          children: [
            if (_imagebytes == null) const CircularProgressIndicator() else Image.memory(_imagebytes!),
            Positioned(
              bottom: 0,
              child: FileDescription(
                description: {
